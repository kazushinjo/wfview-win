...
static void win_average_buffer (double *samples, int sample_count, int half_width);
...
{
    double dB_offset = 7.3, dB_scaler = 64.0, max_dB, min_dB, max_ratio, min_ratio;
    ...
    low_freq = malloc (filtered_count * sizeof (double));
    high_freq = malloc (filtered_count * sizeof (double));
    ...
    for (i = 0; i < filtered_count; ++i)
        if (high_freq [i] > 1.0 && low_freq [i] > 1.0) {
            double ratio = high_freq [i] / low_freq [i];
            int shaping_value;
...
static void win_average_buffer (double *samples, int sample_count, int half_width)
{
    double *output = malloc (sample_count * sizeof (double));
    double sum = 0.0;
    int m = 0, n = 0;
    int i, j, k;
    ...
    memcpy (samples, output, sample_count * sizeof (double));
    free (output);
}
