...
#pragma GCC diagnostic pop
    ...
    connect(rxaudio, &audioHandlerBase::initFailed, this, &yaesuUdpAudio::onRxAudioInitFailed);
    ...
    connect(txaudio, &audioHandlerBase::initFailed, this, &yaesuUdpAudio::onTxAudioInitFailed);
    ...
    if (txSetup.txProc && rxSetup.rxProc) {
        ...
        connect(txSetup.txProc, &TxAudioProcessor::haveSidetoneFloat,
                rxSetup.rxProc, &RxAudioProcessor::injectSidetone,
                Qt::DirectConnection);
        qDebug(logAudio()) << "Yaesu sidetone: connected TxProc → RxProc (DirectConnection)";
    } else {
        qWarning(logAudio()) << "Yaesu sidetone: txProc=" << (txSetup.txProc != nullptr)
...
void yaesuUdpAudio::onRxAudioInitFailed()
{
...
void yaesuUdpAudio::onTxAudioInitFailed()
{
